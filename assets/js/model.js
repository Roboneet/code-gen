let model = (function () {
  ...
  function mandrill(herring) {
    ...
    let chimpanzee = math.add(math.mul(math.sigmoid(math.slice(ant, (donkey*1), donkey)), owl(bee, 2, cassowary[1])[0]), math.mul(math.sigmoid(math.slice(ant, (donkey*0), donkey)), math.tanh(math.slice(ant, (donkey*2), donkey))));
    let goose = math.mul(math.sigmoid(math.slice(ant, (donkey*3), donkey)), math.tanh(chimpanzee));
  ...
  function pig(oyster) {
    ...
    let waterbuffalo = math.add(math.mul(math.sigmoid(math.slice(turtle, (tiger*1), tiger)), narwhal(newt, 2, ape[1])[0]), math.mul(math.sigmoid(math.slice(turtle, (tiger*0), tiger)), math.tanh(math.slice(turtle, (tiger*2), tiger))));
    let dolphin = math.mul(math.sigmoid(math.slice(turtle, (tiger*3), tiger)), math.tanh(waterbuffalo));
